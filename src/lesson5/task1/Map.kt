...
fun subtractOf(a: MutableMap<String, String>, b: Map<String, String>): Unit {
    for ((keyA, _) in b)
        if (a[keyA] == b[keyA]) a.remove(keyA)
}
...
fun canBuildFrom(chars: List<Char>, word: String): Boolean {
    val charsChanged = chars.map { it.toLowerCase() }
    val setOfChars = charsChanged.toSet()
...
fun hasAnagrams(words: List<String>): Boolean {
    val listOfSet = mutableSetOf<Set<Char>>()
    for (elements in words)
        listOfSet.add(elements.toSet())
    return listOfSet.size != words.size
}
...
fun findSumOfTwo(list: List<Int>, number: Int): Pair<Int, Int> {
    ...
    for (i in 0 until list.size)
        differences[i] = number - list[i]

    for (i in 0 until list.size)
        if (differences.containsValue(list[i]))
            for ((key, value) in differences)
                if (value == list[i] && key != i) return Pair(i, key)
